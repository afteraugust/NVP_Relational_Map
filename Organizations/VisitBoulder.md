---
type: organization
status: planning
lead: "[[Savannah Kruger]]"
partners:
  - "[[]]"
funded-by:
  - "[[]]"
tags:
  - funder
  - organization
---

## Description
What this project is and what it aims to accomplish:
Visit Boulder (Boulder CVB) runs the Events, Arts & Culture Grant Program (EAC) in partnership with Create Boulder. Funds events and cultural experiences that strengthen Boulder's brand for visitors and residents. Potential funding source for NVP community-building events; also a connector to Boulder's broader tourism and community vitality ecosystem.

## Partners & Stakeholders
Who's involved and what they contribute:
[[Emiliano Lake-Herrera]] - Vice President, Visitor Experience & Community Partnerships  
	[emiliano.lake-herrera@bouldercvb.com](mailto:emiliano.lake-herrera@bouldercvb.com)  
	303-442-2911, ext. 133

Charlene Hoffman — CEO of Visit Boulder. City of Boulder — Visit Boulder operates as a tourism arm with close city ties (board includes ex-officio city representatives).

## Status & Next Steps
Current state and upcoming milestones:
- Savannah emailed Emiliano on 3/25/26 to ask about grant award amounts (not published anywhere). 
- Kenzie + Sav applying to EAC Grant in late March 2026. 


- Potential longer-term play: positioning NVP as social infrastructure that makes Boulder a stronger destination — aligns with their Sustainable Tourism Plan goals.
